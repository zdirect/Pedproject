...
abstract class BaseModelMethods{
    ...
    protected function createOrder($set, $table = false){
        ...
        if (is_array($set['fields']) && !empty($set['order'])){
    ...
    protected function createWhere($set, $table = false, $instraction = 'WHERE'){
        $table = $table ? $table . '.' : '';
        ...
        if (is_array($set['where']) && !empty($set['where'])){
            ...
            $where = substr($where, 0, intval(strrpos($where, $condition)-1));
